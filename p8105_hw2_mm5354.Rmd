...
Trash_Wheel = 
  readxl::read_excel("./HealthyHarborWaterWheelTotals2017-9-26.xlsx", sheet = 1, range = ("A2:N258")) %>% 
  janitor::clean_names() %>% 
...
```
This dataset is initially not tidy at all, because "Total" this variable/column contains missing values (NAs). So my data cleaning step included omitting NAs, and I also cleaned variable names. Number of observations in both resulting datasets: `r nrow(Trash_Wheel)` for Mr. Trash Wheel sheet and `r nrow(Join_data)` for precipitation data for 2016 and 2017 in total. For available data,the total precipitation in 2017 is `r sum(Precipitation2017noNa$total)` The median number of sports balls in a dumpster in 2016 is `r median(na.omit(cbind(Trash_Wheel$sports_balls[Trash_Wheel$year == 2016])))`.
...
brfss_responseNY = spread(brfss, key = "response", value = "data_value") %>% 
  mutate(., proportion_excellent = (Excellent)/(Excellent + `Very good` + Fair + Good + Poor)) %>% 
